...
Add boxes
'''
def box_annotator( im_path: str,path: str,Same):
...
# Stack into one large dask.array
   ...
   # print(size_pictures)
    ...
    for index in range(SHAPE_STACK[0]):
      box_mouse0.append(np.array([[ index,  SHAPE_STACK[1]/2
                                   ,  SHAPE_STACK[0]/2 ],
         [   index        ,  SHAPE_STACK[1]/2,  3*SHAPE_STACK[0]],
         [   index        , 0.75*SHAPE_STACK[1],  3*SHAPE_STACK[0]],
         [   index        , 0.75*SHAPE_STACK[1],  SHAPE_STACK[0]/2]]))
      box_mouse1.append(np.array([[ index,  SHAPE_STACK[1]/2
                                   ,  SHAPE_STACK[0]/2 ],
         [   index        ,  SHAPE_STACK[1]/2,  3*SHAPE_STACK[0]],
         [   index        , 0.75*SHAPE_STACK[1],  3*SHAPE_STACK[0]],
         [   index        , 0.75*SHAPE_STACK[1],  SHAPE_STACK[0]/2]]))
      # box_mouse0.append(np.array([[ index,  879.52867728
      #                              ,  592.9339079 ],
...
def ConverionTwoPandastoText(data_frame_0, data_frame_1):
    ...
    for index in range(len(data_frame_0.index)):
        ...
        if SAME == False:
            ...
            any_row_as_string_1a = '0'+' '+ any_row_as_string_1
